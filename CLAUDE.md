...
- **侧边栏分组自动排序**：登录时经 HKCU Run 键 `claude-sidebar-sort` 跑 `tools/sort-sidebar-groups.ps1`（💻 Local 组在前、🌐 服务器组在后，块内保序）。原理与限制：分组顺序存在 `Claude-3p\claude_desktop_config.json` 的 `epitaxyPrefs["dframe-group-scopes"]`，App **仅启动时读取**，运行中外部改写无效且会被内存态覆盖——所以只能登录时（App 启动前）排；当天新增的组先追加末尾，下次启动归位。
