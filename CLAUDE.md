...
- **分组命名规则**：本机目录 → `💻 <项目目录名>（Local）`；服务器目录 → `☁️ <项目目录名>（<host>）`（host 用服务器简称，如 `HA`；括号为全角）。目录名取默认自动分组显示的项目名（通常是仓库/目录 basename，worktree 归主仓库名）。例：`💻 待办（Local）`、`☁️ BioData（HA）`。组名是纯文本不支持颜色，用 emoji 做视觉标记。
