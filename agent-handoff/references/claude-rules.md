...
Generated Claude Code rules use the shared maintenance policy: snapshot soft limit `16 KiB / 240 lines`, hard limit `32 KiB / 400 lines`; work log `64 KiB / 30` dated sections; validation `64 KiB / 200` rows; backlog and risks `32 KiB`; archive chunks `128 KiB`. After handoff updates, run the installed skill's `scripts/maintain_handoff.py --repo <repo> --compact-if-needed` when available. The optional hook only warns; it never runs Python or writes state. Preserve unparseable state for manual repair.
